...
namespace PhysicsEngine
{
	...
	public:
		Plane(PxVec3 normal=PxVec3(0.f, 1.f, 0.f), PxReal distance=0.f)
			: StaticActor(PxTransformFromPlaneEquation(PxPlane(normal, distance))) //A plane with default paramters: XZ plane centred at (0,0,0)
		{
	...
	{
	public:
		Sphere(const PxTransform& pose=PxTransform(PxIdentity), PxReal radius=2.0f, PxReal density=1.f) 
			: DynamicActor(pose)
		{ 
	...
	public:
		...
		PxConvexMesh* CookMesh(const PxConvexMeshDesc& mesh_desc) //Mesh cooking (preparation)
		{
	...
	public:
		...
		PxTriangleMesh* CookMesh(const PxTriangleMeshDesc& mesh_desc) //Mesh cooking (preparation)
		{
			PxDefaultMemoryOutputStream stream;
	...
	class DistanceJoint : public Joint //Distance joint with the springs switched on
	{
	public:
